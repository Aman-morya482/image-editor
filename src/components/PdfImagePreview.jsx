...
const PdfPreview = (prop) => {
  ...
  const {images,setImages,login,cancelLogin,pdf,setPdf, createPdf,downloadPdf,remove} = prop;
    ...
    <div className='w-full md:w-3/4 mx-auto'>
        ...
        <div className='relative flex flex-col md:justify-between bg-white items-center px-5 lg:p-30 py-50 overflow-hidden mt-10'>
        ...
        <div>
            ...
            <button className='absolute top-15 right-5 md:right-10 bg-amber-500 text-white rounded-md hover:cursor-pointer ring-amber-300 active:scale-95 hover:ring-3 px-4 py-2 text-xl font-semibold' onClick={pdf? downloadPdf : createPdf}>{pdf ? "Download" : "Create PDF"}</button>
        </div>
        ...
        {!pdf && images && ( 
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-6 w-full overflow-x-scroll border-gray-200 border-2 p-5 rounded-md">
           {images.map((image, index) => (
            ...
            <p className='absolute top-0 right-0 bg-red-400 hover:bg-red-500 text-white px-2 py-1 cursor-pointer' onClick={()=>{remove((index))}}><TiDeleteOutline size={25}/></p>
            </div>
          ))}
        </div>
        )}
