            ...
            <div className="w-full flex flex-col gap-6 px-2 md:px-10 h-auto">
              {prop.compress.map((image, index) => (
                <div key={index} className='boxRef2 relative flex justify-evenly items-center border border-gray-200 shrink-0 overflow-hidden rounded-xl'>
                  ...
                  <button onClick={() => prop.download(image, `image-${index + 1}`)} className='cursor-pointer bg-green-500 hover:bg-green-600 text-white px-2 md:px-3 py-6'><p className='hidden md:block'>Download </p><p className='md:hidden'><GoArrowDown size={22} /></p></button>
                </div>
              ))}
            </div>
          </div>
