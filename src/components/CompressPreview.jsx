  ...
  return format64Size(sizeInBytes);
    ...
    <div className='w-full md:w-3/4 mx-auto'>
        ...
        <div className='flex justify-between items-center w-full'>
            ...
            <button className='z-10 bg-green-500 outline-0 text-white rounded-md hover:cursor-pointer ring-green-300 active:scale-95 hover:ring-3 px-4 py-2 md:text-xl' onClick={prop.sendreq}>{prop.loading ? <p>Loading..</p> : <p>Compress</p>}</button>
        </div>
        ...
        <div className='flex justify-between items-center w-full'>
          ...
          <div className="w-full flex flex-col gap-6 px-2 md:px-10 h-auto">
           {prop.compress.map((image, index) => (
            ...
            <button onClick={() => prop.download(image, `image-${index + 1}`)} className='cursor-pointer bg-green-500 hover:bg-green-600 text-white px-2 md:px-3 py-5'><p className='hidden md:block'>Download </p><p className='md:hidden'><GoArrowDown size={22}/></p></button>
            </div> 
          ))}
