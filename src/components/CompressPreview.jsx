        ...
        <div className='flex justify-between items-center w-full'>
          ...
          <div className="w-full flex flex-col gap-6 px-2 md:px-10 h-auto">
           {prop.compress.map((image, index) => (
            ...
            <p>Before: {formatSize(prop.images[index].size)}</p>
            <p>Now: {calculateImageSize(image)}</p>
            </div>
            </div>
            <button onClick={() => prop.download(image, `image-${index + 1}`)} className='cursor-pointer bg-green-500 text-white px-3 py-4'>Download</button>
            </div> 
          ))}
