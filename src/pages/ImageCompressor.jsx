...
const ImageCompressor = () => {
  ...
  const handleImageChange = () => {
    ...
    setLoading(true);
    setTimeout(() => { setImages(files); setCompress([]); setLoading(false) }, 1200);
  }
  ...
  const handleBacktohome = () => { setImages([]) }
  const handleBacktopre = () => { setCompress([]) }
  const cancelLogin = () => { setLogin(false); setImages([]); }

  const download = (base64String, fileName = "image") => {
    const mimeMatch = base64String.match(/^data:(image\/\w+);base64,/);
    if (!mimeMatch) { console.error("Invalid base64 image format."); return; }
      ...
      <div className="max-w-[1800px] w-full min-h-[92vh] bg-green-200 flex justify-centerce items-center">
        {images.length <= 0 && (
          <div className="relative w-full flex flex-col justify-center items-center">
            <span className='hidden xl:block'><img src="/png/022-minimize-1.png" alt="" width={100} className='absolute -top-25 right-55 scale-svg' /></span>
